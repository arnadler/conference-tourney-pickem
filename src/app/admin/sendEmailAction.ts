...
export async function sendStandingsEmail(): Promise<{ success: boolean; error?: string; sent?: number; preview?: string }> {
  ...
  const response = await anthropic.messages.create({
    ...
    messages: [
      {
        role: "user",
        content: `You're writing a cheeky, trash-talky end-of-day recap email for a college basketball conference tournament pick'em competition between friends. Day 2 just wrapped up.
...
${standingsSummary}`,
  ...
  const emailHtml = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; color: #1e293b;">
      <div style="font-size: 28px; margin-bottom: 16px;">🏀 Day 2 Recap</div>
  ...
  const res = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      to: emails,
      subject: "🏀 Day 2 Pick'Em Standings — Who's cooked?",
    html: emailHtml,
    }),
  });
